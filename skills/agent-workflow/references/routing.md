...
`execution.max_attempts` bounds the outer implementation/verification attempts within a writable task (default 2). It does not count the CLI's internal model rounds. The separate worker `budget` is shared across these attempts and explicit followups. On correctable assertion/file-check failure AW resumes the same session only while both budgets remain. Announce the attempt budget before dispatch or include it in the assignment, then report actual attempts and any budget stop.
...
Pass `--delegations N` with the actual number of submitted task turns for this work package (including explicit follow-ups). Default `routing.max_delegations` is 3. At the limit inspect existing evidence and resolve or report the blocker; do not re-label the same package to reset the budget. This host routing rule is distinct from the runtime work-package budget. Independent user tasks can still submit separately. Task usage is retained, including failures, but may be incomplete; do not claim measured token savings. General analytics and feedback compaction remain separate work.
