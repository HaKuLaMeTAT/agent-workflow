...
import {normalizeDelivery,pageResult} from './result.mjs';
...
import {aggregateTelemetry} from './telemetry.mjs';
import {sumCounters,remainingBudget,exhaustedBudget,budgetPolicy} from './budget.mjs';
...
function storedTelemetry(dir,m) {
  const files=m.workspace_owner?fs.readdirSync(dir).filter(n=>/^attempt-\d+$/.test(n)).map(n=>path.join(dir,n,'telemetry.json')):[path.join(dir,'telemetry.json')];
  const values=files.filter(f=>fs.existsSync(f)).map(f=>readJson(f,2*1024*1024));
  if(!values.length)return null;
  return {...aggregateTelemetry(values),counters:sumCounters(values.map(v=>v.counters)),counters_are_observed_lower_bounds:true,attempts:values.length,
    stop_reason:values.findLast(v=>v.stop_reason)?.stop_reason??null,usage_source:values.every(v=>v.usage_complete)?'provider_reported':'partial_or_unreported',
    model_turns_scope:[...new Set(values.map(v=>v.model_turns_scope))],report_path:path.join(dir,'usage.json')};
}
function shortStatus(m){return {task_id:m.task_id,root_task_id:m.root_task_id,parent_id:m.parent_id,sequence:m.sequence??0,role:m.snapshot.role_id,state:m.state,error:m.error??null,model:m.snapshot.model,effort:m.snapshot.effort,
  usage:m.telemetry?.usage??null,usage_complete:m.telemetry?.usage_complete??false,telemetry:m.telemetry??null,budget:m.snapshot.budget??null,remaining_budget:m.snapshot.remaining_budget?remainingBudget(m.snapshot.remaining_budget,m.telemetry?.counters):null,read_mode:m.snapshot.read_mode??null,
  observation_path:m.observation_path??null,
...
async function refreshUnlocked(h,key) {
  ...
  const usage=storedTelemetry(dir,m);if(usage){m.telemetry=usage;atomicJson(path.join(dir,'usage.json'),usage);}
  if(['completed','failed','cancelled','timed_out'].includes(m.state)){if(JSON.stringify(m)!==before)save(h,m);return m;}
  ...
  if(!receipt) {
    ...
    if(fs.existsSync(exitFile)||!alive) {
      ...
      m.observation_path=path.join(dir,'observation.json');atomicJson(m.observation_path,{accepted:false,note:'Provider observation before AW validation; partial or failed work must not be applied',...observed});
      ...
      if(!fs.existsSync(exitFile)){m.state='interrupted';m.error='Runner exited without a final exit record';}
        ...
        if(exit.reason==='timeout'){m.state='timed_out';m.error='deadline_exceeded';}
          ...
          const delivery=normalizeDelivery(observed.data,m.snapshot.role.result_contract);observed.data=delivery.data;observed.delivery??=delivery.delivery;
          const evidenceFile=path.join(dir,'evidence.json');if(fs.existsSync(evidenceFile))observed.provided_evidence=readJson(evidenceFile);
...
export async function submit(h,{role,cwd,raw,requestId,overrides={},parentId=null}) {
  ...
  let prepared=prepareRequest(requestSnapshot,requestRaw);
  return withFileLock(path.join(h.state_dir,'submit.lock'),async()=>{
    ...
    if(parent) {
      ...
      requireValue((previous.snapshot.read_mode??'native')===prepared.request.read_mode,'read_mode_changed','A native session keeps its read mode; do not expose earlier file-tool history to an evidence-only task');
    }
    const chain=all.filter(m=>m.root_task_id===root),used=sumCounters(chain.map(m=>m.telemetry?.counters??{provider_calls:1}));
    const limit=parent?budgetPolicy(prepared.request.budget,parent.snapshot.budget??prepared.request.budget,{lowerOnly:true}):prepared.request.budget;
    requireValue(!exhaustedBudget(limit,used),'budget_exhausted','The work package has exhausted its budget; inspect usage.json and existing results instead of restarting it');
    snapshot={...snapshot,budget:limit,remaining_budget:remainingBudget(limit,used),read_mode:prepared.request.read_mode,request:prepared.request,evidence_bytes:prepared.evidence?.bytes??0};
    requireValue(snapshot.evidence_bytes<=snapshot.remaining_budget.max_total_read_bytes,'read_budget_exceeded','Evidence exceeds the remaining work-package read budget');
    prepared=prepareRequest({...requestSnapshot,...snapshot,cwd:requestSnapshot.cwd},prepared.request);
    ...
    if(prepared.evidence)atomicJson(path.join(dir,'evidence.json'),prepared.evidence);
...
export async function result(h,key,cursor){return withTaskLock(h,key,async()=>{
  ...
  return {...shortStatus(m),...pageResult(stored,m.snapshot.limits,cursor),usage:m.telemetry?.usage??stored.usage,usage_complete:m.telemetry?.usage_complete??stored.usage_complete??false,
    estimated_cost_usd:m.telemetry?.estimated_cost_usd??stored.estimated_cost_usd??null,
    delivery:stored.delivery??null,...(stored.execution?{execution:stored.execution,usage_scope:'sum of observed provider attempts in this task'}:{}),report_path:path.join(taskDir(h,key),'result.json')};
});}
