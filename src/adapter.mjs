...
export function prepareRequest(snapshot,raw) {
  ...
  const prompt=[
    ...
    snapshot.provider.adapter==='opencode'&&snapshot.role.permissions!=='full-access'?'Use read for declared files/directories and edit/write for the declared write scope. glob/grep/list and terminal tools are disabled because their permissions cannot enforce this path scope. Do not retry a permission denial.':'',
