...
export async function discover(h,providerId,{cwd=process.cwd(),catalog=false,refresh=false}={}) {
  ...
  const fingerprint=hash({provider,cwd:real(cwd),binary_stat:[stat.ino,stat.size,stat.mtimeMs],catalog,path:process.env.PATH,node:process.version});
...
export async function prepareRole(h,snapshot,{refresh=false}={}) {
  ...
  else if(snapshot.role.access!=='read-only'||snapshot.role.can_delegate||!discovery.capabilities?.read_only)reason='permission_unsupported';
  ...
  return {...snapshot,provider:{...snapshot.provider,executable:discovery.executable??snapshot.provider.executable},runnable:reason===null,unavailable_reason:reason,discovery:{...discovery,model_count:discovery.models.length,models:model?[model]:[],selection_verification:model?.verified?'cli_catalog':'configured_or_cached; not server-validated'},
    guarantee:discovery.guarantee??'Capability not established'};
}
export function prepareRequest(snapshot,raw) {
  fields(raw,['goal','acceptance','read_paths','evidence','stage','source','timeout_seconds'],'request');
  ...
  requireValue(['independent','cross-review'].includes(raw.stage??'independent'),'invalid_input','Worker stage must be independent or cross-review');
  ...
  const request={...raw,read_paths:paths,stage:raw.stage??'independent',evidence:raw.evidence??[],source:raw.source??'local'};text(request.source,'source',100);
  const prompt=[
