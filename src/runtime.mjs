...
export function checkRuntime(directory) {
  try {
    ...
    requireValue(marker.patch_id==='aw-prepared-v1','upstream_unpatched','Apply aw-prepared-v1 first');
    requireValue(Object.keys(marker.sha256??{}).sort().join(',')==='dist/cli-process-service.js,dist/detached-runner.cjs','upstream_unpatched','Patch file list mismatch');
