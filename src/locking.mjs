...
export async function withFileLock(file,fn) {
  requireValue(process.platform==='linux','platform_unverified','Task locking currently supports Linux/WSL');
  fs.mkdirSync(path.dirname(file),{recursive:true,mode:0o700});
  const fd=fs.openSync(file,'a',0o600);fs.closeSync(fd);
  const child=spawn('flock',['--exclusive','--timeout','10',file,process.execPath,path.join(import.meta.dirname,'lock-holder.mjs')],{stdio:['pipe','pipe','pipe']});
  child.stdin.on('error',()=>{});
  const closed=new Promise(resolve=>child.once('close',code=>resolve(code)));
  try {
    await new Promise((resolve,reject)=>{
      let ready=false;
      child.once('error',e=>reject(new AwError('lock_unavailable',`Cannot start flock: ${e.code}`)));
