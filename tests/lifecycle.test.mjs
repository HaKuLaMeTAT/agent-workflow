...
function fixture(t) {
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'aw-process-'));
  const runtime=path.join(root,'upstream');fs.cpSync(upstream,runtime,{recursive:true});
  assert.equal(applyUpstreamPatch(runtime).status,'applied');assert.equal(applyUpstreamPatch(runtime).status,'already_applied');
  const binary=path.join(root,'provider');fs.copyFileSync(path.join(TOOL_ROOT,'tests/fixtures/provider.mjs'),binary);fs.chmodSync(binary,0o700);
  ...
  t.after(()=>{
    ...
    if(fs.existsSync(tasks))for(const entry of fs.readdirSync(tasks)) {
      ...
      if(sameProcess(receipt.identity))process.kill(receipt.identity.pid,'SIGTERM');
    }
...
async function childPid(root) {
  for(let i=0;i<50;i++){const f=path.join(root,'descendant.pid');if(fs.existsSync(f))return Number(fs.readFileSync(f,'utf8'));await sleep(100);}
...
test('real runner: caller exit, idempotency, native followup and configuration snapshot',options,async t=>{
  ...
  const call=()=>promisify(execFile)(process.execPath,[path.join(TOOL_ROOT,'bin/aw.mjs'),'run','--host-config',file,'--role','reviewer','--cwd',root,'--request-file',req,'--request-id','one'],{encoding:'utf8',timeout:10000});
...
test('real runner: cancellation and deadline terminate the CLI process group',options,async t=>{
  ...
  const second=await submit(h,{role:'reviewer',cwd:root,raw:request('hang',1),requestId:'timeout'});
...
test('real runner: invalid final output fails; stale PID is observed without signalling',options,async t=>{
  ...
  const holder=spawn(process.execPath,['--input-type=module','-e',`import {withFileLock} from ${JSON.stringify(path.join(TOOL_ROOT,'src/locking.mjs'))};await withFileLock(process.argv[1],async()=>{console.log('ready');await new Promise(()=>{});});`,path.join(h.state_dir,'submit.lock')],{stdio:['ignore','pipe','pipe']});
